...
namespace eboatwright {
    public class Player : GameObject {
        ...
        public const float MOVE_SPEED = 1f, FRICTION = 0.7f, GRAVITY = 0.4f, JUMP_HEIGHT = -7.2f, COYOTE_TIME = 8;
        ...
        public override void Update(float deltaTime, MouseState mouse, KeyboardState keyboard) {
            ...
            Rectangle PlayerRectangle = new Rectangle((int)Math.Floor(position.X), (int)Math.Floor(position.Y), COLLISION_WIDTH - 1, COLLISION_HEIGHT);
            ...
            for (int y = 0; y <= map.mapValues.GetUpperBound(0); y++)
                for (int x = 0; x <= map.mapValues.GetUpperBound(1); x++) {
                    if (map.mapValues[y, x] > 0 && map.mapValues[y, x] < 14) {
                        Rectangle TileRectangle = new Rectangle(x * Map.TILE_SIZE, y * Map.TILE_SIZE, Map.TILE_SIZE, Map.TILE_SIZE);
                        if (RectanglesOverlap(PlayerRectangle, TileRectangle)) {
                            if (velocity.Y > 0f) {
                                velocity.Y = 0f;
            ...
            PlayerRectangle = new Rectangle((int)Math.Floor(position.X), (int)Math.Floor(position.Y), COLLISION_WIDTH, COLLISION_HEIGHT - 1);
